...
class iOSBuilder {
  ...
  async ensureCredentialsAsync() {
    ...
    const bundleIdentifier = await (0, _ios().getBundleIdentifier)(this.ctx.projectDir, this.ctx.exp);
  ...
  async setupAsync() {
    if (this.buildProfile.workflow === _easJson().Workflow.Generic) {
      var _this$buildProfile$sc;

      this.scheme = (_this$buildProfile$sc = this.buildProfile.scheme) !== null && _this$buildProfile$sc !== void 0 ? _this$buildProfile$sc : await this.resolveScheme();
    }
  }
  ...
  async resolveScheme() {
    const schemes = _config().IOSConfig.Scheme.getSchemesFromXcodeproj(this.ctx.projectDir);
